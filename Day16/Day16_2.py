...
# top edge
for i in tqdm(range(col_id)):
    # os.system("clear")
    current_energized = 0
    start_tile = next((t for t in tiles if t.row == 0 and t.col == i), None)
...
for i in tqdm(range(col_id)):
    # os.system("clear")
    current_energized = 0
    start_tile = next((t for t in tiles if t.row == row_id - 1 and t.col == i), None)
    start_tile.is_energized == True
...
# left edge
for i in tqdm(range(row_id)):
    # os.system("clear")
    current_energized = 0
    start_tile = next((t for t in tiles if t.row == i and t.col == 0), None)
    start_tile.is_energized == True
...
for i in tqdm(range(row_id)):
    # os.system("clear")
    current_energized = 0
    start_tile = next((t for t in tiles if t.row == i and t.col == col_id - 1), None)
    start_tile.is_energized == True
